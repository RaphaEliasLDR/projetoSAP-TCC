...
class TelaStatus(MDScreen):
    ...
    def criar_card_pedido_pronto(self, pedido):
        # Ajustando o card para um tamanho menor e mais compacto
        card = MDCard(
            orientation='vertical',
            padding=dp(8),
            spacing=dp(6),
            size_hint_y=None,
            height=dp(160),  # Card menor
            md_bg_color=get_color_from_hex("#C8E6C9"),  # Cor de fundo verde clara
            radius=[8, 8, 8, 8],  # Bordas mais suaves
            elevation=8  # Reduzindo a elevação para um visual mais discreto
        )
        ...
        content_layout = BoxLayout(
            orientation='vertical',
            size_hint_y=None,
            height=dp(140),  # Diminuindo a altura do conteúdo dentro do card
            spacing=dp(8),
            padding=dp(8),
            pos_hint={'center_x': 0.5}
        )

        # Ajuste para o label "Mesa" com font menor
        lbl_mesa = MDLabel(
